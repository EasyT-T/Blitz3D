...
static BOOL FAR PASCAL enumConnection(LPCGUID guid, LPVOID conn, DWORD size, LPCDPNAME name, DWORD flags,
                                      LPVOID context) {
    IDirectPlay4 *dp;
...
static bool startGame(HWND hwnd) {
    ...
    if (!n) {
        MessageBox(hwnd, "Please enter a name for the new game", "DirectPlay Request",
                   MB_SETFOREGROUND | MB_TOPMOST | MB_ICONINFORMATION | MB_OK);
        return false;
    }
        ...
        case WM_INITDIALOG:
            ...
            if (openDirPlay(hwnd)) {
                if (dirPlay->EnumConnections(0, enumConnection, 0, 0) < 0) {
                    MessageBox(hwnd, "Failed to enumerate connections", "DirectPlay Error",
                               MB_SETFOREGROUND | MB_TOPMOST | MB_ICONWARNING | MB_OK);
                }
                closeDirPlay(hwnd);
            }
