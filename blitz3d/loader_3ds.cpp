...
static void parseAnimKeys(Animation* anim, const int type)
{
    ...
    for (int k = 0; k < cnt; ++k)
    {
        ...
        case 0xb022: //SCL_TRACK_TAG
            ...
            _log("SCL_KEY: time=" + itoa(time) + " scale=" + ftoa(scale.x) + "," + ftoa(scale.y) + "," +
                ftoa(scale.z));
...
static void parseMeshInfo(MeshModel* root, float curr_time)
{
    ...
    while (const int chunk_id = nextChunk())
    {
        ...
        case 0xb014: //BOUNDBOX
            ...
            _log("BOUNDBOX: min=" + ftoa(box.a.x) + "," + ftoa(box.a.y) + "," + ftoa(box.a.z) + " max=" +
                ftoa(box.b.x) + "," + ftoa(box.b.y) + "," + ftoa(box.b.z));
            break;
        case 0xb020: //POS_TRACK_TAG
        case 0xb021: //ROT_TRACK_TAG
