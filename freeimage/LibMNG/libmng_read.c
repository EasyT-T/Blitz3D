...
mng_retcode process_eof(mng_datap pData) {
    if (!pData->bEOF)                    /* haven't closed the stream yet ? */
    {
        ...
        if (!pData->fClosestream((mng_handle) pData)) MNG_ERROR (pData, MNG_APPIOERROR)
    }
    ...
    if (pData->bSuspensionmode) {
        ...
        *iRead = 0;                        /* let's be negative about the outcome */
        ...
        if (iSize > pData->iSuspendbufsize) {
            ...
            /* let's go get it */
            if (!pData->fReaddata(((mng_handle) pData), pData->pReadbufnext, iRemain, &iTemp)) MNG_ERROR (pData,
                                                                                                          MNG_APPIOERROR);
            /* first read after suspension return 0 means EOF */
            if ((pData->iSuspendpoint) && (iTemp == 0)) {                                /* that makes it final */
        ...
        } else {                                  /* need to read some more ? */
            while ((!pData->bSuspended) && (!pData->bEOF) && (iSize >
                                                              pData->iSuspendbufleft)) {                                /* not enough space left in buffer ? */
                ...
                if (pData->iSuspendbufsize - pData->iSuspendbufleft < MNG_SUSPENDREQUESTSIZE) MNG_ERROR (pData,
                                                                                                         MNG_INTERNALERROR)
                /* now read some more data */
                pTemp = pData->pSuspendbufnext + pData->iSuspendbufleft;

                if (!pData->fReaddata(((mng_handle) pData), pTemp, MNG_SUSPENDREQUESTSIZE, &iTemp)) MNG_ERROR (pData,
                                                                                                               MNG_APPIOERROR);
                /* adjust fill-counter */
            ...
            if ((!pData->bSuspended) && (!pData->bEOF)) {                                /* return the data ! */
        ...
        if (!pData->fReaddata(((mng_handle) pData), (mng_ptr) pBuf, iSize, iRead)) {
            if (iRead == 0)                  /* suspension required ? */
                pData->bSuspended = MNG_TRUE;
            else MNG_ERROR (pData, MNG_APPIOERROR);
        }
    }
...
/*      if ((!iRetcode) && (!pData->bTimerset) && (pData->bNeedrefresh))
        ...
        if (pData->iBreakpoint)            /* do we need to finish something first ? */
        {
            switch (pData->iBreakpoint)      /* return to broken display routine */
            {
                ...
                case 2 : {
                    iRetcode = process_display_ihdr(pData);
                    break;
                }
                case 3 :;                     /* same as 4 !!! */
                case 4 : {
...
#ifdef MNG_INCLUDE_JNG
                case 7 : {
                    iRetcode = process_display_jhdr(pData);
                    break;
                }
#endif
                case 6 :;                     /* same as 8 !!! */
    ...
    {                                    /* freezing in progress ? */
        ...
        {                                  /* previously suspended or not eof ? */
            if ((pData->iSuspendpoint > 2) ||
                (iRead == iBuflen)) {                                /* determine length chunkname + data + crc */
...
mng_retcode read_graphic(mng_datap pData) {
    ...
    if ((!pData->bHavesig) || (pData->iSuspendpoint == 1)) {
        ...
        if (pData->bSuspended)             /* input suspension ? */
            ...
            else if (mng_get_uint32(pData->pReadbuf) == JNG_SIG)
                pData->eSigtype = mng_it_jng;
            else if (mng_get_uint32(pData->pReadbuf) == MNG_SIG)
                pData->eSigtype = mng_it_mng;
            else MNG_ERROR (pData, MNG_INVALIDSIG);
            /* all of it ? */
            if (mng_get_uint32(pData->pReadbuf + 4) != POST_SIG) MNG_ERROR (pData, MNG_INVALIDSIG);
