...
void check_update_region(mng_datap pData) {                                      /* determine actual canvas row */
...
#endif
    /* viewable row ? */
    if ((pData->iRow >= pData->iSourcet) &&
        (pData->iRow < pData->iSourceb)) {                                    /* address destination row */
        ...
        if (pData->bIsOpaque)              /* forget about transparency ? */
        {
            ...
            {
                for (iX = pData->iSourcel + pData->iCol; iX <
                                                         pData->iSourcer; iX += pData->iColinc) {                              /* scale down by dropping the LSB */
                    *pScanline = *pDataline;
                ...
                for (iX = pData->iSourcel + pData->iCol;
                     iX < pData->iSourcer; iX += pData->iColinc) {                              /* copy the values */
                    *pScanline = *pDataline;
...
mng_retcode display_rgba8(mng_datap pData) {
    ...
    if ((pData->iRow >= pData->iSourcet) &&
        ...
        if (pData->bIsOpaque)              /* forget about transparency ? */
        {
            ...
            {
                for (iX = pData->iSourcel + pData->iCol;
                     iX < pData->iSourcer; iX += pData->iColinc) {                              /* get alpha values */
                    ...
                    {                            /* fully opaque or background fully transparent ? */
                        if ((iFGa16 == 0xFFFF) || (iBGa16 == 0)) {                          /* plain copy it */
                            *pScanline = *pDataline;
                            *(pScanline + 1) = *(pDataline + 2);
                            *(pScanline + 2) = *(pDataline + 4);
                            *(pScanline + 3) = *(pDataline + 6);
                ...
                for (iX = pData->iSourcel + pData->iCol; iX < pData->iSourcer; iX += pData->iColinc) {
                    ...
                    {                            /* fully opaque or background fully transparent ? */
                        if ((iFGa8 == 0xFF) ||
                            (iBGa8 == 0)) {                          /* then simply copy the values */
                            *pScanline = *pDataline;
                            *(pScanline + 1) = *(pDataline + 1);
                            *(pScanline + 2) = *(pDataline + 2);
                            *(pScanline + 3) = *(pDataline + 3);
...
mng_retcode display_rgb8_a8(mng_datap pData) {
    ...
    /* viewable row ? */
    if ((pData->iRow >= pData->iSourcet) &&
        (pData->iRow < pData->iSourceb)) {                                    /* address destination rows */
        pScanline = (mng_uint8p) pData->fGetcanvasline(((mng_handle) pData),
                                                       pData->iRow + pData->iDestt -
                                                       pData->iSourcet);
...
mng_retcode display_bgr8(mng_datap pData) {
    ...
    if ((pData->iRow >= pData->iSourcet) &&
        ...
        if (pData->bIsOpaque)              /* forget about transparency ? */
        {
            ...
            {
                for (iX = pData->iSourcel + pData->iCol;
                     iX < pData->iSourcer; iX += pData->iColinc) {                              /* get alpha value */
                    iA16 = mng_get_uint16(pDataline + 6);
    ...
    if ((pData->iRow >= pData->iSourcet) &&
        ...
        if (pData->bIsOpaque)              /* forget about transparency ? */
        {
            if (pData->bIsRGBA16)            /* 16-bit input row ? */
            {
                ...
                for (iX = pData->iSourcel + pData->iCol; iX <
                                                         pData->iSourcer; iX += pData->iColinc) {                              /* copy the values and premultiply */
                    if ((s = pDataline[3]) == 0)
                        *(mng_uint32 *) pScanline = 0; /* set all components = 0 */
                    else {
                        if (s == 255) {
            ...
            if (pData->bIsRGBA16)            /* 16-bit input row ? */
            {
                for (iX = pData->iSourcel + pData->iCol;
                    ...
                    {                            /* fully opaque or background fully transparent ? */
                        if (s == 255) {                          /* plain copy it */
                            pScanline[0] = pDataline[4];
                            pScanline[1] = pDataline[2];
                            pScanline[2] = pDataline[0];
                            pScanline[3] = 255;
                        } else {                          /* now blend (premultiplied) */
                            t = 255 - s;
                ...
                for (iX = pData->iSourcel + pData->iCol; iX < pData->iSourcer; iX += pData->iColinc) {
                    ...
                    {                            /* fully opaque ? */
                        if (s == 255) {                          /* then simply copy the values */
                            pScanline[0] = pDataline[2];
                            pScanline[1] = pDataline[1];
                            pScanline[2] = pDataline[0];
                            pScanline[3] = 255;
...
mng_retcode restore_bkgd_backcolor(mng_datap pData) {
    ...
    for (iX = pData->iSourcel;
         iX < pData->iSourcer; iX++) {                                    /* ok; drop the background-color in there */
        ...
        case 4 : {
            ...
            if (pBuf->iBitdepth > 8)
                ...
                switch (pBuf->iBitdepth) {
                    case 1 :
                        iGray = (mng_uint8) ((iGray << 1) + iGray);
                    case 2 :
                        iGray = (mng_uint8) ((iGray << 2) + iGray);
                    case 4 :
                        iGray = (mng_uint8) ((iGray << 4) + iGray);
                }
            }
        ...
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            if ((mng_uint16) iG == pBuf->iTRNSgray) {
                ...
                *(pRGBArow + 3) = 0x00;
                ...
                {
                    case 1 :
                        iG = (mng_uint8) ((iG << 1) + iG);
                    case 2 :
                        iG = (mng_uint8) ((iG << 2) + iG);
                    case 4 :
                        iG = (mng_uint8) ((iG << 4) + iG);
                }
...
mng_retcode retrieve_g16(mng_datap pData) {
    ...
    if (pBuf->bHasTRNS)                  /* tRNS in buffer ? */
    {
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            /* is it transparent ? */
            if (iG == pBuf->iTRNSgray) {                                /* nuttin to display */
...
mng_retcode retrieve_rgb16(mng_datap pData) {
    ...
    if (pBuf->bHasTRNS)                  /* tRNS in buffer ? */
    {
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            if ((iR == pBuf->iTRNSred) &&
                (iG == pBuf->iTRNSgreen) &&
                (iB == pBuf->iTRNSblue)) {                                /* nothing to display */
                mng_put_uint16(pRGBArow, 0x0000);
                mng_put_uint16(pRGBArow + 2, 0x0000);
                mng_put_uint16(pRGBArow + 4, 0x0000);
                mng_put_uint16(pRGBArow + 6, 0x0000);
        ...
        for (iX = 0; iX < pData->iRowsamples; iX++) {                                  /* just copy the pixel */
...
mng_retcode retrieve_idx8(mng_datap pData) {
    ...
    if (pBuf->bHasTRNS)                  /* tRNS in buffer ? */
    {
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            /* is it valid ? */
            if ((mng_uint32) iQ < pBuf->iPLTEcount) {                                /* put in intermediate row */
                ...
                *(pRGBArow + 2) = pBuf->aPLTEentries[iQ].iBlue;
                ...
                if ((mng_uint32) iQ < pBuf->iTRNScount)
                    ...
                    *(pRGBArow + 3) = 0xFF;
            } else MNG_ERROR (pData, MNG_PLTEINDEXERROR)
...
mng_retcode store_g16(mng_datap pData) {
    ...
    for (iX = 0; iX < pData->iRowsamples; iX++) {                                    /* copy into object buffer */
...
mng_retcode store_jpeg_g12_a2(mng_datap pData) {
    ...
    for (iX = 0; iX < pData->iRowsamples; iX++) {
        ...
        {
            case 0x03 : {
                mng_put_uint16(pOutrow, 0xFFFF);
                break;
            }
            case 0x02 : {
                mng_put_uint16(pOutrow, 0xAAAA);
                break;
            }
            case 0x01 : {
                mng_put_uint16(pOutrow, 0x5555);
                break;
            }
            default   : {
                mng_put_uint16(pOutrow, 0x0000);
            }
        }
...
mng_retcode store_jpeg_g12_a16(mng_datap pData) {
    ...
    for (iX = 0; iX < pData->iRowsamples; iX++) {                                    /* copy it */
        mng_put_uint16(pOutrow, mng_get_uint16(pWorkrow));
...
mng_retcode delta_g8(mng_datap pData) {
    ...
    if (pData->iDeltatype == MNG_DELTATYPE_BLOCKPIXELREPLACE) {
        ...
        for (iX = 0; iX < pData->iRowsamples; iX++) {                                  /* add to object buffer */
        ...
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            if (iQ == pBuf->iTRNSgray)       /* transparent ? */
                ...
                switch (iQ)                    /* determine the gray level */
                {
                    ...
                    case 0x02 : {
                        mng_put_uint32(pRGBArow, 0xAAAAAAFF);
                        break;
                    }
                    case 0x01 : {
                        mng_put_uint32(pRGBArow, 0x555555FF);
                        break;
                    }
                    default   : {
                        mng_put_uint32(pRGBArow, 0x000000FF);
    ...
    if (pBuf->bHasTRNS)                  /* tRNS encountered ? */
    {
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            if (iQ == pBuf->iTRNSgray)       /* transparent ? */
            {
                ...
                *(pRGBArow + 2) = 0;
                *(pRGBArow + 3) = 0;
            } else {                                /* expand to 8-bit by replication */
                iQ = (mng_uint8) (iQ + (iQ << 4));
...
mng_retcode process_rgb16(mng_datap pData) {
    ...
    if (pBuf->bHasTRNS)                  /* tRNS encountered ? */
    {
        for (iX = 0; iX < pData->iRowsamples; iX++) {
            ...
            /* transparent ? */
            if ((iR == pBuf->iTRNSred) && (iG == pBuf->iTRNSgreen) &&
                (iB == pBuf->iTRNSblue)) {                                /* transparent then */
                mng_put_uint16(pRGBArow, 0);
                mng_put_uint16(pRGBArow + 2, 0);
                mng_put_uint16(pRGBArow + 4, 0);
                mng_put_uint16(pRGBArow + 6, 0);
        ...
        for (iX = 0; iX < pData->iRowsamples; iX++) {                                  /* copy the RGB values */
            mng_put_uint16(pRGBArow, mng_get_uint16(pWorkrow));
            mng_put_uint16(pRGBArow + 2, mng_get_uint16(pWorkrow + 2));
            mng_put_uint16(pRGBArow + 4, mng_get_uint16(pWorkrow + 4));
            mng_put_uint16(pRGBArow + 6, 0xFFFF);
    ...
    if (pData->pStoreobj)                /* store in object too ? */
    {
        if (pData->iJHDRimgbitdepth == 8) {
            switch (pData->iJHDRcolortype) {
                case 12 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_g8_a1;
                    break;
                }
                case 14 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_rgb8_a1;
                    break;
                }
            }
        }
...
mng_retcode init_jpeg_a2_ni(mng_datap pData) {
    ...
    if (pData->pStoreobj)                /* store in object too ? */
    {
        if (pData->iJHDRimgbitdepth == 8) {
            switch (pData->iJHDRcolortype) {
                case 12 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_g8_a2;
                    break;
                }
                case 14 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_rgb8_a2;
                    break;
                }
            }
        }
...
mng_retcode init_jpeg_a4_ni(mng_datap pData) {
    ...
    if (pData->pStoreobj)                /* store in object too ? */
    {
        if (pData->iJHDRimgbitdepth == 8) {
            switch (pData->iJHDRcolortype) {
                case 12 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_g8_a4;
                    break;
                }
                case 14 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_rgb8_a4;
                    break;
                }
            }
        }
...
mng_retcode init_jpeg_a8_ni(mng_datap pData) {
    ...
    if (pData->pStoreobj)                /* store in object too ? */
    {
        if (pData->iJHDRimgbitdepth == 8) {
            switch (pData->iJHDRcolortype) {
                case 12 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_g8_a8;
                    break;
                }
                case 14 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_rgb8_a8;
                    break;
                }
            }
        }
...
mng_retcode init_jpeg_a16_ni(mng_datap pData) {
    ...
    if (pData->pStoreobj)                /* store in object too ? */
    {
        if (pData->iJHDRimgbitdepth == 8) {
            switch (pData->iJHDRcolortype) {
                case 12 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_g8_a16;
                    break;
                }
                case 14 : {
                    pData->fStorerow = (mng_fptr) store_jpeg_rgb8_a16;
                    break;
                }
            }
        }
    ...
    if (pData->iPass >= 0)               /* interlaced ? */
    {
        while ((pData->iPass < 7) &&       /* went 'outside' the image ? */
            ...
            if ((pData->iPass < 7) &&        /* reset previous row to zeroes ? */
                (pData->iRow < (mng_int32) pData->iDataheight) &&
                (pData->iCol <
                 (mng_int32) pData->iDatawidth)) {                                /* making sure the filters will work properly! */
                mng_int32 iX;
...
mng_retcode next_jpeg_row(mng_datap pData) {
    ...
    {                                    /* has alpha channel ? */
        if ((pData->iJHDRcolortype == MNG_COLORTYPE_JPEGGRAYA) ||
            (pData->iJHDRcolortype ==
             MNG_COLORTYPE_JPEGCOLORA)) {                                  /* try to display what you can */
            iRetcode = display_jpeg_rows(pData);
        } else {                                  /* make sure we all know which row to handle */
    ...
    for (iX = 0; iX < iWidth; iX++) {
        *pTempdst = *pTempsrc1;            /* copy original source pixel */
        ...
        if (iX == 0)                       /* first interval ? */
            iM = iML;
        else if (iX == (iWidth - 1))            /* last interval ? */
            iM = iMR;
        else
            iM = iMX;
    ...
    for (iX = 0; iX < iWidth; iX++) {
        ...
        *pTempdst = *pTempsrc1;            /* copy original source pixel */
        ...
        if (iX == 0)                       /* first interval ? */
        {
            ...
            iM = iML;
        } else if (iX == (iWidth - 2))            /* last interval ? */
            iM = iMR;
        else
            iM = iMX;
        /* fill interval ? */
...
mng_retcode magnify_g8_y2(mng_datap pData,
    ...
    {
        for (iX = 0; iX < iWidth; iX++) {                                  /* calculate the distances */
            if (*pTempsrc1 == *pTempsrc2)
                *pTempdst = *pTempsrc1;
            else
                *pTempdst = (mng_uint8) (((2 * iS * ((mng_int32)(*pTempsrc2) -
    ...
    } else {                                    /* just repeat the entire line */
...
mng_retcode magnify_g8_y3(mng_datap pData,
    ...
    if (pSrcline2)                       /* do we have a second line ? */
    {
        ...
        MNG_COPY (pDstline, pSrcline1, iWidth)
        else MNG_COPY (pDstline, pSrcline2, iWidth)
    } else {                                    /* just repeat the entire line */
        MNG_COPY (pDstline, pSrcline1, iWidth)
    }
...
mng_retcode magnify_rgb8_y3(mng_datap pData,
    ...
    if (pSrcline2)                       /* do we have a second line ? */
    {
        if (iS < (iM + 1) / 2)               /* top half ? */
        MNG_COPY (pDstline, pSrcline1, iWidth * 3)
        else MNG_COPY (pDstline, pSrcline2, iWidth * 3)
    } else {                                    /* just repeat the entire line */
        MNG_COPY (pDstline, pSrcline1, iWidth * 3)
    }
...
mng_retcode magnify_ga8_y3(mng_datap pData,
    ...
    if (pSrcline2)                       /* do we have a second line ? */
    {
        if (iS < (iM + 1) / 2)               /* top half ? */
        MNG_COPY (pDstline, pSrcline1, iWidth << 1)
        else MNG_COPY (pDstline, pSrcline2, iWidth << 1)
    } else {                                    /* just repeat the entire line */
        MNG_COPY (pDstline, pSrcline1, iWidth << 1)
    }
...
mng_retcode magnify_rgba8_y3(mng_datap pData,
    ...
    if (pSrcline2)                       /* do we have a second line ? */
    {
        if (iS < (iM + 1) / 2)               /* top half ? */
        MNG_COPY (pDstline, pSrcline1, iWidth << 2)
        else MNG_COPY (pDstline, pSrcline2, iWidth << 2)
    } else {                                    /* just repeat the entire line */
        MNG_COPY (pDstline, pSrcline1, iWidth << 2)
    }
