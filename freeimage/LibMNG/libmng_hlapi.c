...
mng_retcode MNG_DECL mng_read_resume(mng_handle hHandle) {
    ...
    if ((!pData->bReading) || (!pData->bSuspended)) MNG_ERROR (pData, MNG_FUNCTIONINVALID)
...
mng_retcode MNG_DECL mng_readdisplay(mng_handle hHandle) {
    ...
    /* valid at this point ? */
    if ((pData->bReading) || (pData->bDisplaying)) MNG_ERROR (pData, MNG_FUNCTIONINVALID)
    ...
    if (pData->bSuspended)               /* read suspension ? */
    {
        iRetcode = MNG_NEEDMOREDATA;
        pData->iSuspendtime = pData->fGettickcount((mng_handle) pData);
    } else if (pData->bTimerset)                /* indicate timer break ? */
        iRetcode = MNG_NEEDTIMERWAIT;
    else if (pData->bSectionwait)             /* indicate section break ? */
...
#ifdef MNG_SUPPORT_READ
    if (pData->bReading) MNG_ERROR (pData, MNG_FUNCTIONINVALID)
#endif
...
mng_retcode MNG_DECL mng_display_freeze(mng_handle hHandle) {
    ...
    pData = ((mng_datap) hHandle);        /* and make it addressable */
    /* can we expect this call ? */
    if ((!pData->bDisplaying) || (pData->bReading)) MNG_ERROR (pData, MNG_FUNCTIONINVALID)
...
mng_retcode MNG_DECL mng_display_goframe(mng_handle hHandle,
    ...
    /* can we expect this call ? */
    if ((!pData->bDisplaying) || (pData->bRunning)) MNG_ERROR ((mng_datap) hHandle, MNG_FUNCTIONINVALID)
...
mng_retcode MNG_DECL mng_display_golayer(mng_handle hHandle,
    ...
    MNG_ERROR (pData, MNG_NOTANANIMATION)
    /* can we expect this call ? */
    if ((!pData->bDisplaying) || (pData->bRunning)) MNG_ERROR (pData, MNG_FUNCTIONINVALID)
