...
mng_retcode set_delay(mng_datap pData,
    ...
    if (!pData->fSettimer((mng_handle) pData, iInterval)) MNG_ERROR (pData, MNG_APPTIMERERROR)
    ...
    if (!pData->bSearching)              /* we mustn't be searching !!! */
    {
        if ((pData->bRunning) &&           /* let the app refresh first ? */
            ...
            if (!pData->fRefresh(((mng_handle) pData),
                                 ...
                                 pData->iUpdatebottom - pData->iUpdatetop)) MNG_ERROR (pData, MNG_APPMISCERROR)
            ...
            /* interval requested ? */
            if ((!pData->bFreezing) && (iInterval)) {                                /* setup the timer */
...
void set_display_routine(mng_datap pData) {                                        /* actively running ? */
    if ((pData->bRunning) && (!pData->bFreezing)) {
        switch (pData->iCanvasstyle)         /* determine display routine */
        {
            case MNG_CANVAS_RGB8    : {
                pData->fDisplayrow = (mng_fptr) display_rgb8;
                break;
            }
            case MNG_CANVAS_RGBA8   : {
                pData->fDisplayrow = (mng_fptr) display_rgba8;
                break;
            }
            case MNG_CANVAS_ARGB8   : {
                pData->fDisplayrow = (mng_fptr) display_argb8;
                break;
            }
            case MNG_CANVAS_RGB8_A8 : {
                pData->fDisplayrow = (mng_fptr) display_rgb8_a8;
                break;
            }
            case MNG_CANVAS_BGR8    : {
                pData->fDisplayrow = (mng_fptr) display_bgr8;
                break;
            }
            case MNG_CANVAS_BGRA8   : {
                pData->fDisplayrow = (mng_fptr) display_bgra8;
                break;
            }
            case MNG_CANVAS_BGRA8PM : {
                pData->fDisplayrow = (mng_fptr) display_bgra8_pm;
                break;
            }
            case MNG_CANVAS_ABGR8   : {
                pData->fDisplayrow = (mng_fptr) display_abgr8;
                break;
            }
    ...
    if ((pData->bRunning) && (!pData->bFreezing)) {
        ...
        if ((pData->iDestr >= pData->iDestl) && (pData->iDestb >= pData->iDestt)) {
            ...
            if (((pData->eImagetype == mng_it_png) || (pData->eImagetype == mng_it_jng)) &&
                (pData->bUseBKGD)) {                                /* prefer bKGD in PNG/JNG */
            ...
            if (pData->fGetbkgdline)         /* background-canvas-access callback set ? */
            {
                switch (pData->iBkgdstyle) {
                    case MNG_CANVAS_RGB8    : {
                        pData->fRestbkgdrow = (mng_fptr) restore_bkgd_rgb8;
                        break;
                    }
                    case MNG_CANVAS_BGR8    : {
                        pData->fRestbkgdrow = (mng_fptr) restore_bkgd_bgr8;
                        break;
                    }
            ...
            if (pData->bHasBACK) {                                /* background image ? */
         ...
         it probably requires a rewrite of the logic here... */
            ...
            while ((!iRetcode) && (iY < pData->iDestb)) {                                /* restore a background row */
...
mng_retcode clear_canvas(mng_datap pData) {
    ...
    while ((!iRetcode) && (iY < pData->iDestb)) {                                    /* clear a row then */
        iRetcode = ((mng_displayrow) pData->fDisplayrow)(pData);
...
/* changed here because FRAM 1/3 will delay themselves before each image */
            if ((pData->iFrameseq) && (iFramemode != 1) && (iFramemode != 3))
                ...
                pData->iFramedelay = pData->iNextdelay;
        } else {                                  /* delay before inserting background layer? */
...
mng_retcode next_layer(mng_datap pData) {
    ...
    if (!pData->bTimerset)               /* timer still off ? */
    {
        ...
        {                                  /* wait till IDAT/JDAT for PNGs & JNGs !!! */
            ...
            else {                                /* for MNG we do it right away */
                iRetcode = load_bkgdlayer(pData);

                if (pData->bRunning)
                    pData->iLayerseq++;          /* and it counts as a layer then ! */
            }
        } else if (pData->iFramemode == 3)        /* restore background for each layer ? */
            iRetcode = load_bkgdlayer(pData);
        ...
        if ((pImage->pImgbuf->iWidth) &&
            (pImage->pImgbuf->iHeight)) {                                  /* and maximum size  */
        ...
        if (!pData->bTimerset)             /* all systems still go ? */
        {
            ...
            if ((pData->iDestr >= pData->iDestl) && (pData->iDestb >= pData->iDestt)) {
                ...
                switch (pImage->pImgbuf->iColortype) {
                    ...
                    case 3 : {
                        pData->fRetrieverow = (mng_fptr) retrieve_idx8;
                        pData->bIsOpaque = (mng_bool) (!pImage->pImgbuf->bHasTRNS);
                        break;
                    }
                ...
                while ((!iRetcode) && (iY < pData->iSourceb)) {                              /* get a row */
    ...
    if ((pData->bRunning) && (!pData->bFreezing)) {
        ...
        if ((!pData->bDeltaimmediate) && (pData->iDeltatype != MNG_DELTATYPE_NOCHANGE)) {
                    ...
                    if ((pData->iDeltatype == MNG_DELTATYPE_REPLACE) ||
                        ...
                        if ((pBufdelta->iColortype == 0) || (pBufdelta->iColortype == 3) ||
                            ...
                            switch (pBuftarget->iBitdepth) {
                                case 1 : {
                                    pData->fDeltarow = (mng_fptr) delta_g1_g1;
                                    break;
                                }
                                case 2 : {
                                    pData->fDeltarow = (mng_fptr) delta_g2_g2;
                                    break;
                                }
                                case 4 : {
                                    pData->fDeltarow = (mng_fptr) delta_g4_g4;
                                    break;
                                }
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_g8_g8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_g16_g16;
                                    break;
                                }
                            }
                        }
                ...
                case 10 : {                     /* rgb */
                    if ((pData->iDeltatype == MNG_DELTATYPE_REPLACE) ||
                        ...
                        if ((pBufdelta->iColortype == 2) || (pBufdelta->iColortype == 10)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgb8_rgb8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgb16_rgb16;
                                    break;
                                }
                            }
                        }
                ...
                case 12 : {                     /* gray + alpha */
                    if ((pData->iDeltatype == MNG_DELTATYPE_REPLACE) ||
                        ...
                        if ((pBufdelta->iColortype == 4) || (pBufdelta->iColortype == 12)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_ga8_ga8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_ga16_ga16;
                                    break;
                                }
                            }
                        }
                    } else if ((pData->iDeltatype == MNG_DELTATYPE_BLOCKCOLORADD) ||
                               (pData->iDeltatype == MNG_DELTATYPE_BLOCKCOLORREPLACE)) {
                        if ((pBufdelta->iColortype == 0) || (pBufdelta->iColortype == 3) ||
                            (pBufdelta->iColortype == 8)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_ga8_g8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_ga16_g16;
                                    break;
                                }
                            }
                        }
                    } else if ((pData->iDeltatype == MNG_DELTATYPE_BLOCKALPHAADD) ||
                               (pData->iDeltatype == MNG_DELTATYPE_BLOCKALPHAREPLACE)) {
                        if ((pBufdelta->iColortype == 0) || (pBufdelta->iColortype == 3)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_ga8_a8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_ga16_a16;
                                    break;
                                }
                            }
                        }
                ...
                case 14 : {                     /* rgb + alpha */
                    if ((pData->iDeltatype == MNG_DELTATYPE_REPLACE) ||
                        ...
                        if ((pBufdelta->iColortype == 6) || (pBufdelta->iColortype == 14)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgba8_rgba8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgba16_rgba16;
                                    break;
                                }
                            }
                        }
                        ...
                        if ((pBufdelta->iColortype == 2) || (pBufdelta->iColortype == 10)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgba8_rgb8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgba16_rgb16;
                                    break;
                                }
                            }
                        }
                        ...
                        if ((pBufdelta->iColortype == 0) || (pBufdelta->iColortype == 3)) {
                            switch (pBuftarget->iBitdepth) {
                                case 8 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgba8_a8;
                                    break;
                                }
                                case 16 : {
                                    pData->fDeltarow = (mng_fptr) delta_rgba16_a16;
                                    break;
                                }
                            }
                        }
            ...
            if (pData->fDeltarow)            /* do we need to take action ? */
            {
                ...
                while ((!iRetcode) && (iY < pBufdelta->iHeight)) {                              /* get a row */
            ...
            } else MNG_ERROR (pData, MNG_INVALIDDELTA)
...
mng_retcode save_state(mng_datap pData) {
    ...
    while (pImage) {                                    /* freeze the object AND it's buffer */
    ...
    do                                   /* process the objects */
    {
        ...
        if ((pData->iBreakpoint) && (pData->iBreakpoint < 99)) {
            switch (pData->iBreakpoint)      /* return to broken display routine */
            {
                ...
                case 5  : {
                    iRetcode = process_display_clon2(pData);
                    break;
                }
                case 9  : {
                    iRetcode = process_display_magn2(pData);
                    break;
                }
...
mng_retcode
process_display_ihdr(mng_datap pData) {                                      /* address the current "object" if any */
    mng_imagep pImage = (mng_imagep) pData->pCurrentobj;
    ...
    if (!pData->iBreakpoint)             /* not previously broken ? */
    {
        ...
        if (pData->bHasDHDR)               /* is a delta-image ? */
        {
            ...
            else if ((pData->iDeltatype == MNG_DELTATYPE_BLOCKPIXELADD) ||
                     (pData->iDeltatype == MNG_DELTATYPE_BLOCKPIXELREPLACE)) {
...
mng_retcode process_display_iend(mng_datap pData) {
    ...
    if ((pData->bHasBASI) ||             /* was it a BASI stream */
        ...
        if (!pImage)                       /* or was it object 0 ? */
            pImage = (mng_imagep) pData->pObjzero;
        /* display it now then ? */
        if ((pImage->bVisible) && (pImage->bViewable)) {                                  /* ok, so do it */
            ...
            case 3 : {                       /* repeat */
                ...
                {                     /* restore to initial or SAVE state */
                    ...
                    if (pTERM->iDelay)  /* set the delay (?) */
                    {
                        ...
                        if (pData->iTicks) {                 /* honor speed modifier */
                            switch (pData->iSpeed) {
                                case mng_st_fast : {
...
mng_retcode process_display_basi(mng_datap pData,
                                 ...
                                 mng_bool bHasalpha,
                                 mng_uint16 iAlpha,
                                 mng_uint8 iViewable) {                                      /* address the current "object" if any */
    mng_imagep pImage = (mng_imagep) pData->pCurrentobj;
    mng_uint8p pWork;
    mng_uint32 iX;
    mng_imagedatap pBuf;
                ...
                if (iS > 0)                    /* forward ? */
                {
                    for (iX = iFrom; iX <= iTo; iX += iS) {
                        ...
                        if (pImage)                  /* object exists ? */
                        {
                            ...
                            else if (iTrigger)              /* found the trigger ? */
                            {
                                pImage->bVisible = MNG_TRUE;
                                iFound = iX;
                                pFound = pImage;
                            } else if (pImage->bVisible)      /* ok, this is the trigger */
                            {
                                pImage->bVisible = MNG_FALSE;
                                iTrigger = iX;
                            }
...
mng_retcode
process_display_jhdr(mng_datap pData) {                                      /* address the current "object" if any */
    mng_imagep pImage = (mng_imagep) pData->pCurrentobj;
    mng_retcode iRetcode = MNG_NOERROR;
    ...
    if (!pData->bHasDHDR) {                                    /* we're always storing a JPEG */
        if (pImage)                        /* real object ? */
            pData->pStoreobj = pImage;       /* tell the row routines */
        else                               /* otherwise use object 0 */
            pData->pStoreobj = pData->pObjzero;
    ...
    {                                    /* default row initialization ! */
        ...
        if ((!pData->bHasDHDR) ||
            (pData->iDeltatype == MNG_DELTATYPE_REPLACE)) {                                  /* 8-bit JPEG ? */
            if (pData->iJHDRimgbitdepth == 8) {                                /* intermediate row is 8-bit deep */
                pData->bIsRGBA16 = MNG_FALSE;
            ...
            if (pData->iJHDRalphacompression == MNG_COMPRESSION_DEFLATE) {
                ...
                switch (pData->iJHDRalphabitdepth) {
                    case 1 : {
                        pData->fInitrowproc = (mng_fptr) init_jpeg_a1_ni;
                        break;
                    }
                    case 2 : {
                        pData->fInitrowproc = (mng_fptr) init_jpeg_a2_ni;
                        break;
                    }
                    case 4 : {
                        pData->fInitrowproc = (mng_fptr) init_jpeg_a4_ni;
                        break;
                    }
                    case 8 : {
                        pData->fInitrowproc = (mng_fptr) init_jpeg_a8_ni;
                        break;
                    }
                    case 16 : {
                        pData->fInitrowproc = (mng_fptr) init_jpeg_a16_ni;
                        break;
                    }
                }
            } else                             /* possible JDAA alpha-channel ? */
            if (pData->iJHDRalphacompression ==
                MNG_COMPRESSION_BASELINEJPEG) {                                /* 8-bit JPEG ? */
                if (pData->iJHDRimgbitdepth == 8) {
                    if (pData->iJHDRcolortype == MNG_COLORTYPE_JPEGGRAYA)
                        pData->fStorerow3 = (mng_fptr) store_jpeg_g8_alpha;
                    else if (pData->iJHDRcolortype == MNG_COLORTYPE_JPEGCOLORA)
            ...
            if (iRetcode)                    /* on error bail out */
                return iRetcode;
        } else {                                  /* must be alpha add/replace !! */
            if ((pData->iDeltatype != MNG_DELTATYPE_BLOCKALPHAADD) &&
                (pData->iDeltatype != MNG_DELTATYPE_BLOCKALPHAREPLACE)) MNG_ERROR (pData, MNG_INVDELTATYPE)
            /* determine alpha processing routine */
            switch (pData->iJHDRalphabitdepth) {
                case 1 : {
                    pData->fInitrowproc = (mng_fptr) init_g1_ni;
...
mng_retcode process_display_dhdr(mng_datap pData,
    ...
    if (pImage)                          /* object exists ? */
    {
        ...
        {                                  /* previous magnification to be done ? */
            ...
            switch (iDeltatype)              /* determine nr of delta-channels */
            {
                ...
                case MNG_DELTATYPE_BLOCKALPHAREPLACE : {
                    ...
                    } else if ((pData->iColortype == MNG_COLORTYPE_RGBA) ||
                               (pData->iJHDRcolortype == MNG_COLORTYPE_JPEGCOLORA)) {
                        pData->iColortype = MNG_COLORTYPE_GRAY;
                        pData->iJHDRcolortype = MNG_COLORTYPE_JPEGGRAY;
                    }
        ...
        } else MNG_ERROR (pData, MNG_OBJNOTCONCRETE)

    } else MNG_ERROR (pData, MNG_OBJECTUNKNOWN)
...
mng_retcode process_display_magn(mng_datap pData,
    ...
    for (iX = iFirstid; iX <= iLastid; iX++) {
        if (iX == 0)                       /* process object 0 ? */
        {
            ...
            if ((pImage) &&
                (!pImage->bFrozen)) {                                /* previous magnification to be done ? */
                if ((pImage->iMAGN_MethodX) || (pImage->iMAGN_MethodY)) {
