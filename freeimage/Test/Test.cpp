...
//				for (int i = 0; i < sizeof(s_extensions) / sizeof(s_extensions[0]); ++i) {
                    ...
                    static_cast<void * (DLL_CALLCONV * )(FIBITMAP * dib,
                    const char *filename,
                    int flags)>(s_extensions[i].function)(dib, unique, BMP_DEFAULT);
                }
...
main(int argc, char *argv[]) {
    ...
    for (int j = FreeImage_GetFIFCount() - 1; j >= 0; --j)
        printf("bitmap type %d (%s): %s (%s)\n", j, FreeImage_GetFormatFromFIF((FREE_IMAGE_FORMAT) j),
               FreeImage_GetFIFDescription((FREE_IMAGE_FORMAT) j),
               FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT) j));
