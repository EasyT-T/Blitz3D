    ...
    t_surf(0), cm_mask(0), locked_cnt(0), mod_cnt(0), remip_cnt(0)
{
    if (flags & CANVAS_TEX_CUBE)
    {
        ...
        for (int k = 0; k < 6; ++k)
        {
            ...
            switch (k)
            {
            case 0: n = DDSCAPS2_CUBEMAP_NEGATIVEX;
                break;
            case 1: n = DDSCAPS2_CUBEMAP_POSITIVEZ;
                break;
            case 2: n = DDSCAPS2_CUBEMAP_POSITIVEX;
                break;
            case 3: n = DDSCAPS2_CUBEMAP_NEGATIVEZ;
                break;
            case 4: n = DDSCAPS2_CUBEMAP_POSITIVEY;
                break;
            case 5: n = DDSCAPS2_CUBEMAP_NEGATIVEY;
                break;
            default: return;
            }
            DDSCAPS2 caps = {0};
...
void gxCanvas::line(int x0, int y0, int x1, int y1)
{
    ...
    while (true)
    {
        ...
        if (x0 > cx1)clip0 |= 4;
        else if (x0 < cx0)clip0 |= 8;
        if (y1 > cy1)clip1 |= 1;
        else if (y1 < cy0)clip1 |= 2;
        if (x1 > cx1)clip1 |= 4;
        else if (x1 < cx0)clip1 |= 8;
...
bool gxCanvas::rect_collide(int x1, int y1, int x2, int y2, int w2, int h2, bool solid) const
{
    ...
    y1 -= handle_y;
    if (y1 + clip_rect.bottom <= y2 || y1 >= y2 + h2) return false;
...
{
    x += origin_x;
    if (x < viewport.left || x >= viewport.right) return format.toARGB(mask_surf);
    y += origin_y;
...
void gxCanvas::copyPixel(int x, int y, gxCanvas* src, int src_x, int src_y)
{
    ...
    if (y < viewport.top || y >= viewport.bottom) return;
    src_x += src->origin_x;
    if (src_x < src->viewport.left || src_x >= src->viewport.right) return;
    src_y += src->origin_y;
    if (src_y < src->viewport.top || src_y >= src->viewport.bottom) return;
    lock();
    src->lock();
