...
{
    return CreateDirectory(dir.c_str(), nullptr) ? true : false;
}
...
{
    const DWORD t = GetFileAttributes(name.c_str());
...
gxDir* gxFileSystem::openDir(const std::string& name, int flags)
{
    ...
    WIN32_FIND_DATA f;
    const HANDLE h = FindFirstFile(t.c_str(), &f);
...
{
    return dir_set.count(d) ? d : nullptr;
}
